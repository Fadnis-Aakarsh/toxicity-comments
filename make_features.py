...
class BinaryProcessor(DataProcessor):
    ...
    def convert_text_to_examples(self, input_texts=[]):
        """ Input format - [{ 'text': 'your input example', 'label': '0/1' or None, 'id': '1232' }]"""
        """ text is compulsory, id and label are optional """
        examples = []
        alpha = 'a'
        for i, text_obj in enumerate(input_texts):
            rand_id = str(int(time.time() * 10000)) + ':' + str(randint(1000, 1000000))
            label = None
            text = text_obj.get('text')
            if text_obj.get('id'):
                rand_id = text_obj.get('id')

            if text_obj.get('label') and text_obj.get('label') is not None:
                label = str(text_obj.get('label'))

            if text is not None and len(text.strip()) > 0:
                text = text.strip()
            else:
                continue
            examples.append([rand_id, label, alpha, text])
        return self._create_examples(examples, 'text')
